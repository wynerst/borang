...
if (INDEX_AUTH != 1) { die("can not access this file directly"); }
...
if ($unauthorized) {
  ...
  $msg .= 'top.location.href = \'./login.php\';'."\n";
  $msg .= '</script>'."\n";
...
if ($timeout) {
  $msg = '<div class="alert alert-error">';
  $msg .= 'Your Login Session has already timeout! <a target="_top" href="./login.php">Re-Login</a>';
  $msg .= '</div>'."\n";
  // unset cookie admin flag
  setcookie('admin_logged_in', true, time()-86400, SENAYAN_WEB_ROOT_DIR);
  simbio_security::destroySessionCookie($msg, SENAYAN_SESSION_COOKIES_NAME, SENAYAN_WEB_ROOT_DIR, true);
