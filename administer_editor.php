...
//display data not reviewed
   ...
   $where = '';
   if ($desa >> 0) { $where .= ' AND c.idlokasi='.$desa; }
   if ($filter >> 0) { $where .= ' AND r.idkel_ruas='.$filter; }
   if ($tahun >> 0) { $where .= ' AND c.tahun='.$tahun; }

	$main_sql = $main_sql.$where;
	...
	if ($jumlah_hasil_temuan == 0) {
		...
		$config['url_page'] = 'administer_editor.php?filter='.$filter.'&tahun='.$tahun.'&id='.$desa.'&page=';
