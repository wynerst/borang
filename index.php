...
echo '<form method="GET" class="form-search">';
/** echo '<select name="id">';
...
*/
if (isset($tahun) AND $tahun == "") {
	echo '<input type="text" name="thn" placeholder="Periode Tahun" class="input-small"> ';
} else {
	echo '<input type="text" name="thn" value="'.$tahun.'" class="input-small"> ';
}
...
echo '<button type="submit" class="btn btn-success">Filter</button>';
echo '</form>';
echo '</div>';
echo '</div>';
...
if (isset($_GET['filter'])) {
	...
	$main_sql = 'SELECT c.*, r.nama, l.nama as desa, u.realname FROM main_cerita as c LEFT JOIN ruas as r ON c.idruas = r.idruas
